...
!---------------------------------------------
! Method I - Jacobi Solver  !!!!!!!!!!!!!!!!
!---------------------------------------------

  do k = 0, n_it-1  !  computes up to n_it because k+1, boundary only set to n_it.
     do initU_i = 1, N
        do j = 1, N
           U(k+1,initU_i,j) = 0.25*(u(k,initU_i-1,j)+u(k,initU_i+1,j)+u(k,initU_i,j-1)+u(k,initU_i,j+1))-0.25*hIN*hIN*F(initU_i,j)
        enddo
     enddo
  enddo
  ...
  do k = 0, n_it-1
     do initU_i = 1, N
       do j = 1, N
          GS=0.25*(u(k+1,initU_i-1,j)+u(k+1,initU_i,j-1)+u(k,initU_i+1,j)+u(k,initU_i,j+1))-0.25*hIN*hIN*F(initU_i,j)
          U_hold = w*(GS-u(k,initU_i,j))
          U(k+1,initU_i,j) = u(k,initU_i,j)+ U_hold
      enddo
    enddo
  enddo
  !-------------------------------------------
